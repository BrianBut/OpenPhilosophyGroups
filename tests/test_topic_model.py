...
class TopicModelTestCase(unittest.TestCase):
    ...
    def test_topic_is_private(self):
        t = Topic(title="Test Topic", published = 2)
        db.session.add(t)
        db.session.commit()
        #print('discussion_venue: {}'.format(t.discussion_venue() ))
        self.assertTrue(t.discussion_venue() == 'private')
    ...
    def test_admin_topic_author_full_name(self):
        ...
        t=topic.dump()
        print(t)
        self.assertFalse(t['author_fullname'] == 'Admin Lee')
        self.assertTrue(t['author_fullname']=='_')
    '''