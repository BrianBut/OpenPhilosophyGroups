...
from . import auth
from .. import db
from ..models import User, Role, MailList, Permission
...
def login():
    ...
    if form.validate_on_submit():
        ...
        logger.info('Login attempt from {}'.format( form.email.data ))
        if user is not None and user.verify_password(form.password.data):
            login_user(user, form.remember_me.data)
            logger.info('Login succeded for {}'.format( user.email ) )
            next = request.args.get('next')
            if next is None or not next.startswith('/'):
                next = url_for('main.index')
            return redirect(next)
        flash(category='danger', message='Invalid email or password.')
        logger.info('Login failure by {}'.format( form.email.data ))
...
def register():
    ...
    if form.validate_on_submit():
        logger.info('Received registration request from {}'.format( form.email.data ))
        user = User(email=form.email.data.lower(), password=form.password.data, name='_'.join( [ form.first_name.data, form.last_name.data ] ))
        db.session.add(user)
        db.session.commit()
        token = user.generate_confirmation_token()
        logger.info('Attempting to send confirmation message to {}'.format( user.email ))
